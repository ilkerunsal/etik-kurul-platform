...
- `POST /applications/{id}/committee-review/request-revision`, `POST /applications/{id}/committee-review/approve` ve `POST /applications/{id}/committee-review/reject` endpoint'leri `secretariat` rolune aciktir; kurul kararini audit kaydi olarak saklar ve basvuruyu sirasiyla `CommitteeRevisionRequested`, `Approved` veya `Rejected` adimina tasir.
- `POST /applications/{id}/committee-revision-response` endpoint'i basvuru sahibine aciktir; son kurul revizyon talebine yanit kaydi olusturur ve basvuruyu tekrar `UnderCommitteeReview` adimina tasir.
...
- Uygulama acilisinda relational veritabani mevcut olup yeni `applications`, `application_expert_assignments`, `application_expert_review_decisions`, `application_revision_responses`, `application_review_packages`, `application_committee_agenda_items`, `application_committee_decisions` veya `application_committee_revision_responses` semasi eksikse, yalnizca bu izole stack icin sema yeniden kurulur.
